...
def test_observe_profile_determinism(monkeypatch: object, tmp_path: Path) -> None:
    ...
    for index in [1, 2]:
        ...
        profile_sha = hashlib.sha256((run_dir / "observe" / "observe.profile.json").read_bytes()).hexdigest()
        manifest_hash = _canonical_hash(run_dir / "observe" / "observe.profile.manifest.json")
        hashes.append((profile_sha, manifest_hash))
