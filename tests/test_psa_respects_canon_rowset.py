...
def test_psa_respects_canon_rowset(monkeypatch: object, tmp_path: Path) -> None:
    ...
    for policy in ["full", "rth", "eth"]:
        run_dir = tmp_path / f"run_{policy}"
        ...
        canon_df = pd.read_parquet(run_dir / "canon.parquet")
        psa_df = pd.read_parquet(run_dir / "psa.parquet")

        assert len(canon_df) == len(psa_df)
        assert canon_df["ts"].tolist() == psa_df["ts"].tolist()
