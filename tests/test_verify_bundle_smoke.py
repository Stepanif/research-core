...
def test_verify_bundle_smoke(monkeypatch: object, tmp_path: Path) -> None:
    ...
    verify_result = runner.invoke(app, ["verify", "bundle", "--zip", str(bundle_path)])
    assert verify_result.exit_code == 0, verify_result.stdout
    assert "RESEARCH DOCTOR v1" in verify_result.stdout
    assert "HASH INTEGRITY" in verify_result.stdout
    assert "[FAIL]" not in verify_result.stdout
