...
def test_plan_env_fingerprint_determinism(monkeypatch: object, tmp_path: Path) -> None:
    ...
    project_path.write_text(
        json.dumps(
            {
                "project_version": "v1",
                "name": "plan-env-fingerprint-determinism",
                "runs": ["run_a"],
                "spec_dirs": ["specs"],
    ...
    h1 = sha256_json(read_json(fingerprint_path))

    second_execute = runner.invoke(
        app,
        ["plan", "execute", "--plan", str(plan_path), "--jobs", "1", "--allow-existing"],
    )
    assert second_execute.exit_code == 0, second_execute.stdout

    h2 = sha256_json(read_json(fingerprint_path))
    assert h1 == h2
