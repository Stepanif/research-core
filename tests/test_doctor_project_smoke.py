...
def test_doctor_project_smoke(monkeypatch: object, tmp_path: Path) -> None:
    ...
    list_result = runner.invoke(app, ["project", "list", "--output-dir", str(output_dir)])
    assert list_result.exit_code == 0, list_result.stdout
    project_id = sorted([path.name for path in output_dir.iterdir() if path.is_dir()])[0]

    promote_result = runner.invoke(
        app,
        ["project", "promote", "--output-dir", str(output_dir), "--id", project_id, "--label", "prod"],
    )
    assert promote_result.exit_code == 0, promote_result.stdout

    doctor_result = runner.invoke(
        app,
        ["doctor", "project", "--output-dir", str(output_dir), "--id", project_id],
    )
    assert doctor_result.exit_code == 0, doctor_result.stdout
    assert "RESEARCH DOCTOR v1" in doctor_result.stdout
    assert "RESULT" in doctor_result.stdout
    assert "[FAIL]" not in doctor_result.stdout
