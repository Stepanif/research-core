...
def test_risk_sweep_smoke(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_spec = tmp_path / "runset_risk_sweep_smoke.json"
    runset_spec.write_text(
        json.dumps(
            {
                "runset_version": "v1",
                "name": "runset-risk-sweep-smoke",
                "datasets": [dataset_id],
                "runs": [
                    {
                        "run_ref": "run",
    ...
    assert (output_root / "baseline.card.json").exists()
    assert (output_root / "baseline.card.manifest.json").exists()
