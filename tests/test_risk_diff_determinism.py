...
def test_risk_diff_determinism(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_spec = tmp_path / "runset_risk_diff_determinism.json"
    runset_spec.write_text(
        json.dumps(
            {
                "runset_version": "v1",
                "name": "runset-risk-diff-determinism",
                "datasets": [dataset_id],
                "runs": [
                    {
                        "run_ref": "run",
    ...
    diff_hash_1 = hashlib.sha256(diff_path.read_bytes()).hexdigest()
    manifest_1 = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_hash_1 = manifest_1["baseline_diff_manifest_canonical_sha256"]
    ...
    diff_hash_2 = hashlib.sha256(diff_path.read_bytes()).hexdigest()
    manifest_2 = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_hash_2 = manifest_2["baseline_diff_manifest_canonical_sha256"]

    assert diff_hash_1 == diff_hash_2
