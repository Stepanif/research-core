...
def test_doctor_run_detects_corruption(monkeypatch: object, tmp_path: Path) -> None:
    ...
    result = runner.invoke(app, ["doctor", "run", "--run", str(run_dir)])
    assert result.exit_code != 0
    assert "RESEARCH DOCTOR v1" in result.stdout
    assert "[FAIL]" in result.stdout
    assert "manifest.observe_summary.output.observe.summary.json" in result.stdout
