...
def test_ci_run_determinism(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_id = create_runset(runner, tmp_path, dataset_id, canon_hash, "runset-ci-det")
    ...
    one = runner.invoke(app, ["ci", "run", "--config", "ci.json"])
    ...
    two = runner.invoke(app, ["ci", "run", "--config", "ci.json"])
