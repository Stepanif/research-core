...
def _prepare_catalog_and_spec(monkeypatch: object, tmp_path: Path) -> tuple[CliRunner, Path]:
...
def test_runset_immutability_fail_loud(monkeypatch: object, tmp_path: Path) -> None:
    runner, spec_path = _prepare_catalog_and_spec(monkeypatch=monkeypatch, tmp_path=tmp_path)
    ...
    entry_payload["datasets"] = sorted(set(entry_payload["datasets"] + ["mutated-dataset-id"]))
    entry_path.write_text(json.dumps(entry_payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False) + "\n", encoding="utf-8")
