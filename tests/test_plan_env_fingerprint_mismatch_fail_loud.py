...
def test_plan_env_fingerprint_mismatch_fail_loud(monkeypatch: object, tmp_path: Path) -> None:
    ...
    project_path.write_text(
        json.dumps(
            {
                "project_version": "v1",
                "name": "plan-env-fingerprint-mismatch",
                "runs": ["run_a"],
                "spec_dirs": ["specs"],
    ...
    fingerprint_path = plan_path.parent / "env.fingerprint.json"
    payload = read_json(fingerprint_path)
    payload["python"]["version"] = "0.0.0"
    fingerprint_path.write_text(json.dumps(payload, sort_keys=True, separators=(",", ":"), ensure_ascii=False) + "\n", encoding="utf-8")

    second_execute = runner.invoke(
        app,
        ["plan", "execute", "--plan", str(plan_path), "--jobs", "1", "--allow-existing"],
    )
    assert second_execute.exit_code != 0
    message = str(second_execute.exception or second_execute.stdout).lower()
