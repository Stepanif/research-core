...
def test_risk_sweep_determinism(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_spec = tmp_path / "runset_risk_sweep_determinism.json"
    runset_spec.write_text(
        json.dumps(
            {
                "runset_version": "v1",
                "name": "runset-risk-sweep-determinism",
                "datasets": [dataset_id],
                "runs": [
                    {
                        "run_ref": "run",
    ...
    output_root = tmp_path / "risk_out" / runset_id
    card_path = output_root / "baseline.card.json"
    manifest_path = output_root / "baseline.card.manifest.json"

    card_hash_1 = hashlib.sha256(card_path.read_bytes()).hexdigest()
    manifest_1 = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_canonical_hash_1 = manifest_1["baseline_card_manifest_canonical_sha256"]
    ...
    card_hash_2 = hashlib.sha256(card_path.read_bytes()).hexdigest()
    manifest_2 = json.loads(manifest_path.read_text(encoding="utf-8"))
    manifest_canonical_hash_2 = manifest_2["baseline_card_manifest_canonical_sha256"]

    assert card_hash_1 == card_hash_2
    assert manifest_canonical_hash_1 == manifest_canonical_hash_2
