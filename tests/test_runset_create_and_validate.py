...
def _build_lineage_ready_run(runner: CliRunner, repo_root: Path, tmp_path: Path, dataset_id: str) -> tuple[Path, str]:
    ...
    assert isinstance(canon_hash, str) and canon_hash
    return run_dir, canon_hash


def test_runset_create_and_validate(monkeypatch: object, tmp_path: Path) -> None:
    ...
    run_dir, canon_hash = _build_lineage_ready_run(runner, repo_root=repo_root, tmp_path=tmp_path, dataset_id=dataset_id)
    ...
    validate = runner.invoke(app, ["runset", "validate", "--catalog", "catalog", "--id", runset_id])
    assert validate.exit_code == 0, validate.stdout
    assert f"PASS runset_id={runset_id}" in validate.stdout
