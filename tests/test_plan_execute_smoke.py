...
def test_plan_execute_smoke(monkeypatch: object, tmp_path: Path) -> None:
    ...
    execute_result = runner.invoke(app, ["plan", "execute", "--plan", str(plan_path), "--jobs", "2"])
    assert execute_result.exit_code == 0, execute_result.stdout

    payload = read_json(plan_path)
    for task in payload["tasks"]:
        run_dir_abs = Path(task["argv"][task["argv"].index("--run") + 1])
        for rel_path in task["expected_outputs"]:
            assert (run_dir_abs / rel_path).exists()
