...
def test_plan_execute_allow_existing_fails_on_incomplete(monkeypatch: object, tmp_path: Path) -> None:
    ...
    project_path.write_text(
        json.dumps(
            {
                "project_version": "v1",
                "name": "plan-allow-existing-incomplete",
                "runs": ["run_a"],
                "spec_dirs": ["specs"],
    ...
    run_dir_abs = Path(task["argv"][task["argv"].index("--run") + 1])
    missing_rel = sorted(task["expected_outputs"])[0]
    missing_path = run_dir_abs / missing_rel
    missing_path.unlink()
    ...
    assert "EXISTS_INCOMPLETE" in second_execute.stdout
