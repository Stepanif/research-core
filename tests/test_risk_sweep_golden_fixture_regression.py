...
def _manifest_canonical_hash(path: Path) -> str:
    ...
    data = {k: v for k, v in payload.items() if k != "baseline_card_manifest_canonical_sha256"}
    return hashlib.sha256(json.dumps(data, sort_keys=True, separators=(",", ":"), ensure_ascii=False).encode("utf-8")).hexdigest()


def test_risk_sweep_golden_fixture_regression(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_spec = tmp_path / "runset_risk_sweep_golden.json"
    runset_spec.write_text(
        json.dumps(
            {
                "runset_version": "v1",
                "name": "runset-risk-sweep-golden",
                "datasets": [dataset_id],
                "runs": [
                    {
                        "run_ref": "run",
    ...
    expected_card_hash = (
        repo_root / "tests" / "golden" / "baseline_card_small_sample_v1.card.json.sha256"
    ).read_text(encoding="utf-8").strip()
    expected_manifest_hash = (
        repo_root / "tests" / "golden" / "baseline_card_small_sample_v1.manifest.json.sha256"
    ).read_text(encoding="utf-8").strip()

    assert card_hash == expected_card_hash
