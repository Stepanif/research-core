...
def test_plan_env_fingerprint_smoke(monkeypatch: object, tmp_path: Path) -> None:
    ...
    project_path.write_text(
        json.dumps(
            {
                "project_version": "v1",
                "name": "plan-env-fingerprint-smoke",
                "runs": ["run_a"],
                "spec_dirs": ["specs"],
    ...
    assert (plan_path.parent / "env.fingerprint.json").exists()
