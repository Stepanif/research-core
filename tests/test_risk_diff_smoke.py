...
def test_risk_diff_smoke(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_spec = tmp_path / "runset_risk_diff_smoke.json"
    runset_spec.write_text(
        json.dumps(
            {
                "runset_version": "v1",
                "name": "runset-risk-diff-smoke",
                "datasets": [dataset_id],
                "runs": [
                    {
                        "run_ref": "run",
    ...
    assert payload["diff_version"] == "v1"
    assert isinstance(manifest["baseline_diff_manifest_canonical_sha256"], str)
