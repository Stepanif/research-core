...
def test_ci_doctor_golden_fixture_regression(monkeypatch: object, tmp_path: Path) -> None:
    ...
    runset_id = create_runset(runner, tmp_path, dataset_id, canon_hash, "runset-ci-doctor-golden")
    ...
    expected_summary_hash = (
        repo_root / "tests" / "golden" / "ci_doctor_small_sample_v1.summary.json.sha256"
    ).read_text(encoding="utf-8").strip()
    expected_manifest_hash = (
        repo_root / "tests" / "golden" / "ci_doctor_small_sample_v1.manifest.json.sha256"
    ).read_text(encoding="utf-8").strip()
