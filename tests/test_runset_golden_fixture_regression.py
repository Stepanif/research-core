...
def test_runset_golden_fixture_regression(monkeypatch: object, tmp_path: Path) -> None:
    ...
    entry_hash = _canonical_hash(tmp_path / "catalog" / "runsets" / "entries" / f"{runset_id}.json")
    index_hash = _canonical_hash(tmp_path / "catalog" / "runsets" / "runsets.index.json")

    expected_entry_hash = (repo_root / "tests" / "golden" / "runset_small_sample_v1.entry.json.sha256").read_text(encoding="utf-8").strip()
    expected_index_hash = (repo_root / "tests" / "golden" / "runset_small_sample_v1.index.json.sha256").read_text(encoding="utf-8").strip()
