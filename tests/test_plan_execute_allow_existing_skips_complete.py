...
def test_plan_execute_allow_existing_skips_complete(monkeypatch: object, tmp_path: Path) -> None:
    ...
    assert "SKIP_EXISTS,0" in second_execute.stdout
    assert "RUN_OK" not in second_execute.stdout
    assert "RUN_FAIL" not in second_execute.stdout
