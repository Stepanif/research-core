...
def test_project_index_golden_fixture_regression(monkeypatch: object, tmp_path: Path) -> None:
    ...
    index_hash = sha256_json(read_json(output_dir / "projects.index.json"))
    expected = Path("tests/golden/projects_index_small_sample_v1.json.sha256").read_text(encoding="utf-8").strip()
    assert index_hash == expected
