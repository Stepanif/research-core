...
def test_project_materialize_determinism(monkeypatch: object, tmp_path: Path) -> None:
    ...
    project_path.write_text(
        json.dumps(
            {
                "project_version": "v1",
                "name": "project-materialize-determinism",
                "datasets": [
                    {
    ...
    summary_path = project_dir / "materialize.summary.json"
    summary_payload_1 = json.loads(summary_path.read_text(encoding="utf-8"))
    summary_hash_1 = hashlib.sha256(summary_path.read_bytes()).hexdigest()
    run_dir_1 = summary_payload_1["runs"][0]["run_dir"]

    r2 = runner.invoke(app, cmd)
    assert r2.exit_code == 0, r2.stdout

    summary_payload_2 = json.loads(summary_path.read_text(encoding="utf-8"))
    summary_hash_2 = hashlib.sha256(summary_path.read_bytes()).hexdigest()
    run_dir_2 = summary_payload_2["runs"][0]["run_dir"]
