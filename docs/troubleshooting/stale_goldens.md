...
In this repo, a stale golden means a test-computed hash no longer matches the
expected hash file in `tests/golden/*.sha256`.
...
- Cause: canonical JSON or hashing behavior changed (contracts require canonical
	JSON in multiple specs, for example baseline diff and dataset catalog).
...
1. Confirm this is an expected behavior change, not nondeterminism.
2. Reproduce with focused failing test(s).
3. Update golden hashes in a controlled, minimal scope.
...
5. Re-run docs generation verification:
...
Already supported in this repo:

- Golden fixtures are hash-based (`tests/golden/*.sha256`) rather than storing
	large full-output snapshots.

TODO: Define a documented, repo-level regolden workflow that updates only the
minimal affected hash files and records which contract change required each update.
