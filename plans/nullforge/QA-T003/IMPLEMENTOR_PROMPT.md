# QA-T003 Implementor Prompt

You are Codex working in `C:\Users\Filip\Desktop\Repos\research-core`.

Task: implement `QA-T003` only. Do not commit.

Context:

- `QA-T003` context and planner artifacts exist under `plans/nullforge/QA-T003/`.
- `QA-T001`, `HY-T001`, and `QA-T002` have audit `PASS`.
- `QA-T002` confirmed a local Python environment and CLI/runtime blocker:
...
- `QA-T003` is docs-only repair decisioning. It must not repair the environment.
...
- `.github/`
- `README.md`
- `docs/` except allowed NullForge status/source-index/QA decision files
- `tools/`
- `src/`
- `tests/`
- `schemas/`
...
- Do not create audits for QA-T003.
...
1. Verify current status and `QA-T001` / `HY-T001` / `QA-T002` audit `PASS`.
2. Create `docs/nullforge/qa/ENVIRONMENT_REPAIR_DECISION.md`.
3. Document candidate repair/readiness paths without executing any repair.
4. State that QA-T003 may only prepare a human gate packet and must not repair the environment.
5. Update `docs/nullforge/CURRENT_STATUS.md` only if needed for QA-T003 decisioning state while preserving `No NullForge implementation code has started.` and `REPO_SOURCE_IMPORT_BASELINE`.
6. Update `docs/nullforge/SOURCE_INDEX.md` only if needed to link the QA-T003 decision doc and reports after creation.
...
- Explicit distinction between source facts, local environment observations, expected unsupported command shapes, unresolved blockers, and candidate repair/readiness paths.
- Explicit statement that no install, uninstall, editable install, dependency sync, package build, virtual-environment work, environment repair, full tests, docs generation, docs build, quickstart, or CI smoke commands were run.
...
- human gates
- whether QA-T003 is ready for independent audit
