...
def _execute_task(task: dict[str, Any], repo_root: Path, logs_dir: Path, env: dict[str, str]) -> TaskResult:
    ...
    if status == "success":
        run_dir_abs = _parse_run_from_argv(argv)
        expected_outputs = _require_list(task, "expected_outputs")
        missing = [
            str(rel_path)
            for rel_path in sorted([str(item) for item in expected_outputs])
            if not (run_dir_abs / str(rel_path)).exists()
        ]
        if missing:
            status = "failed"
            exit_code = 101
...
def execute_plan(plan_path: Path, jobs: int) -> tuple[list[str], bool]:
    if jobs < 1:
    ...
    with ThreadPoolExecutor(max_workers=jobs) as pool:
        while remaining and not failure_detected and len(running) < jobs:
            ...
            future = pool.submit(_execute_task, task, repo_root, logs_dir, env)
            running[future] = task
    ...
    for task in remaining:
        ...
        results[task_id] = TaskResult(task_id=task_id, status="not_launched", exit_code=-1)

    summary_lines = [
        f"{task_id},{results[task_id].status},{results[task_id].exit_code}"
        for task_id in sorted(results.keys())
    ]
    all_ok = all(results[task_id].status == "success" for task_id in sorted(results.keys()))
